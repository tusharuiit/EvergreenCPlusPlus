...
int main ()
{
   ...
   std::cout<<"Maxdepth of tree = "<< maxd << std::endl ;
}
...
int maxdepth ( node * root)
{
    ...
    {
        if ((maxdepth(root->rightnode)) > (maxdepth(root->leftnode)))
        {
           return ( 1 + (maxdepth(root->rightnode)));
        }
        else 
        {
           return ( 1 + (maxdepth(root->leftnode)));
        }
    }
}
